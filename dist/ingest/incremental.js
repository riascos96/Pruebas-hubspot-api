...
async function getWatermark(pg, name) {
...
async function setWatermark(pg, name, ms) {
    await pg.query(`INSERT INTO meta.sync_state(entity,kind,last_updated_at)
     VALUES($1,'object',to_timestamp($2/1000.0))
     ON CONFLICT (entity,kind) DO UPDATE SET last_updated_at=EXCLUDED.last_updated_at`, [name, ms]);
}
...
async function incObject(name) {
    ...
    if (maxSeen > since)
        await setWatermark(pg, name, maxSeen);
    log.info({ name, total }, "INC ok");
}
export default async function main() {
    for (const name of OBJECTS_INCREMENTAL) {
        try {
            await incObject(name);
        }
        catch (e) {
            console.error("INC error", name, e.message);
        }
    }
}
if (import.meta.main)
