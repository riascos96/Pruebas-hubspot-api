...
function resolveSubject(objectName, props) {
    const perObject = {
...
function resolveStatus(objectName, props) {
    const perObject = {
...
function resolveActivityType(objectName, props) {
    const perObject = {
...
function resolveDueAt(objectName, props) {
    const perObject = {
...
export async function normalizeActivities() {
    const pg = getDbConnection();
    const { rows } = await pg.query(`SELECT object_name, hs_object_id, payload, updated_at
       FROM raw_hubspot.objects_raw
      WHERE object_name = ANY($1::text[])`, [ACTIVITY_OBJECTS]);
    let processed = 0;
    for (const row of rows) {
        ...
        await pg.query(`INSERT INTO stg.activities (
