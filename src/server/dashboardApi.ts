...
interface SummaryRow {
  contacts: string | number | null;
  companies: string | number | null;
  activities: string | number | null;
  deals: string | number | null;
  contact_owners: string | number | null;
}
...
type DashboardPayload = {
  ...
  contactsByOwner: Array<{ label: string; count: number }>;
  contactsMonthly: Array<{ label: string; count: number }>;
  companiesByOwner: Array<{ label: string; count: number }>;
  activitiesByType: Array<{ label: string; count: number }>;
  contactsByOwnerStatus: Array<{ owner: string; status: string; count: number }>;
  activitiesMonthly: Array<{ label: string; count: number }>;
};
...
function toMetric(rows: MetricRow[]): Array<{ label: string; count: number }> {
  return rows.map((row) => ({
...
function toOwnerStatus(rows: OwnerStatusRow[]): Array<{ owner: string; status: string; count: number }> {
  return rows.map((row) => ({
...
async function fetchDashboard(): Promise<DashboardPayload> {
  ...
  const [summaryRow, statusRows, ownerRows, monthlyRows, companyRows, activityRows, ownerStatusRows, activitiesMonthlyRows] =
    await Promise.all([
      pg.query<SummaryRow>(
        `SELECT
      ...
      pg.query<MetricRow>(
        `SELECT COALESCE(NULLIF(lead_status, ''), 'Sin estado') AS label, COUNT(*)::int AS count
           FROM stg.contacts
          GROUP BY 1
          ORDER BY count DESC`
    ).then((res) => res.rows),
    pg.query<MetricRow>(
      `SELECT COALESCE(NULLIF(TRIM(CONCAT_WS(' ', mo.first_name, mo.last_name)), ''), mo.email, sc.owner_id, 'No asignado') AS label,
              COUNT(*)::int AS count
         FROM stg.contacts sc
         LEFT JOIN meta.owners mo ON mo.owner_id::text = sc.owner_id
        GROUP BY 1
    ...
    pg.query<MetricRow>(
      `SELECT object_name AS label, COUNT(*)::int AS count
         FROM stg.activities
        GROUP BY 1
        ORDER BY count DESC`
    ).then((res) => res.rows),
      pg.query<OwnerStatusRow>(
        `WITH contacts_enriched AS (
         ...
         SELECT ce.owner_label AS owner,
          ...
          ORDER BY ce.owner_label, ce.status_label`
      ).then((res) => res.rows),
      pg.query<MetricRow>(
        `SELECT to_char(date_trunc('month', created_at), 'YYYY-MM') AS label,
                COUNT(*)::int AS count
...
async function handleDashboard(res: ServerResponse) {
  try {
...
const tidyShutdown = (signal: NodeJS.Signals) => {
