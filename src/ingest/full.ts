...
async function ingestObject(name: string) {
  ...
  while (true) {
    // Paso 1: listar IDs (ligero)
    const page = await hsGetJson(`/crm/v3/objects/${name}`, {
      limit: 100,
      archived: false,
      ...(after ? { after } : {})
    });
    const ids: string[] = (page.results ?? []).map((r: any) => String(r.id));
    // Paso 2: batch read con TODAS las propiedades
    if (ids.length) {
      const body = { inputs: ids.map(id => ({ id })), properties: props } as any;
  ...
  await pg.query(
    `INSERT INTO meta.sync_state(entity,kind,last_after) VALUES($1,'object',NULL)
     ON CONFLICT (entity,kind) DO UPDATE SET last_after=NULL`,
    [name]
  );
...
export default async function main() {
  for (const name of OBJECTS_ALL) {
    try { await ingestObject(name); }
    catch (e) { console.error("FULL error", name, (e as any).message); }
  }
}
