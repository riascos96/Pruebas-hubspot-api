...
async function doFetch(
  ...
  return limiter.schedule(async () => {
    ...
    const signal = init?.signal ? (AbortSignal as any).any?.([init.signal, ac.signal]) ?? ac.signal : ac.signal;
    try {
...
export async function hsGetJson(path: string, params?: Record<string, string | number | boolean | string[]>) {
...
export async function hsPostJson(path: string, body: any) {
  const url = new URL(path, BASE);
